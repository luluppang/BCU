...
class AddMaskPatchTrigger(object):
    def __init__(self, trigger_pattern, trigger_mask):
        self.pattern = trigger_pattern
        self.mask = trigger_mask
        # self.pattern = trigger_array.item().get("pattern")
        # self.mask = trigger_array.item().get("mask")
    ...
    def add_trigger(self, img):
        return img * (1 - self.mask) + self.pattern * self.mask
        #return img * (self.trigger_array == 0) + self.trigger_array * (self.trigger_array > 0)

class AddBlendedPatchTrigger(object):
    def __init__(self, trigger_pattern, trigger_mask):
        self.pattern = trigger_pattern
        self.mask = trigger_mask
        # self.pattern = trigger_array.item().get("pattern")
    ...
    def add_trigger(self, img):
        (x_indices, y_indices, _) = np.nonzero(self.mask)
        x1, x2 = min(x_indices), max(x_indices)
        y1, y2 = min(y_indices), max(y_indices)
        alpha_t = self.pattern[x1:(x2+1), y1:(y2+1), :] / 255.0 * 0.99
        alpha_i = 1.0 - alpha_t
        img[x1:(x2+1), y1:(y2+1), :] = alpha_i * img[x1:(x2+1), y1:(y2+1), :] + alpha_t * self.pattern[x1:(x2+1), y1:(y2+1), :]
        return img
        # return img * (1 - self.mask) + self.pattern * self.mask
        #return img * (self.trigger_array == 0) + self.trigger_array * (self.trigger_array > 0)
