        ...
        if clean_label == False:
            ...
            if p_num is not None or round(pratio * len(targets)):
                if p_num is not None:
                    ...
                    poisoned_counts = round(pratio * len(np.where(targets == tlabel)[0]))
                    non_zero_array = np.random.choice(np.where(targets == tlabel)[0], poisoned_counts, replace=False)
                    pidx[list(non_zero_array)] = 1
...
def generate_pidx_from_label_transform(
    ...
    if isinstance(label_transform, AllToOne_attack):
        ...
        for label in unique_labels:
            label_index = np.where(original_labels == label)[0]
            if p_num is not None:
                select_position = np.random.choice(len(label_index), size=p_num, replace=False)
            elif pratio is not None:
                select_position = np.random.choice(len(label_index), size=round(len(label_index) * pratio), replace=False)
            else:
                raise SystemExit('p_num or pratio must be given')
            pidx[list(label_index[list(select_position)])] = 1

        indicator, indicator_counts = np.unique(pidx, return_counts=True)
        if len(indicator_counts) == 2:
            logging.info(f'poison num:{indicator_counts[1]},real pratio:{indicator_counts[1] / len(original_labels)}')
        else:
            logging.info(f'poison num:{indicator_counts[0]},real pratio:{indicator_counts[0] / len(original_labels)}')
        return pidx
    else:
