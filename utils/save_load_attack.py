    ...
    for k, v in input_dict.items():
        if isinstance(v, dict):
            ...
            if isinstance(v, np.ndarray) and v.shape[0] == 0:
                summary_dict_return[k] = {'shape': v.shape, }
            else:
                summary_dict_return[k] = {'shape':v.shape, 'min':v.min(), 'max':v.max(),}
...
from utils.aggregate_block.dataset_and_transform_generate import dataset_and_transform_generate, dataset_and_transform_generate_diversity
...
def load_attack_result(
    ...
    if all(key in load_file for key in ['model_name',
        ...
        clean_setting.dataset_path = save_path[:save_path.index('code')] + clean_setting.dataset_path[clean_setting.dataset_path.index('data'):]
        ...
        val_dataset_without_transform, val_img_transform, val_label_transform = dataset_and_transform_generate(clean_setting)
        ...
        clean_val_ds = prepro_cls_DatasetBD(
            val_dataset_without_transform,
            poison_idx=np.zeros(len(val_dataset_without_transform)),
            # one-hot to determine which image may take bd_transform
            bd_image_pre_transform=None,
            bd_label_pre_transform=None,
            ori_image_transform_in_loading=val_img_transform,
            ori_label_transform_in_loading=val_label_transform,
            add_details_in_preprocess=True,
        )
        clean_val_x, clean_val_y, _, _, _ = add_resize_and_subset_for_prepro_cls_DatasetBD(clean_val_ds, clean_setting.img_size)
...
def load_attack_result_od(args,
    save_path : str,
):
    '''
    This function first replicate the basic steps of generate models and clean train and test datasets
    ...
    if all(key in load_file for key in ['model_name',
        ...
        train_dataset_without_transform, train_img_transform, train_label_transfrom, \
        test_dataset_without_transform, test_img_transform, test_label_transform, \
        _, val_img_transform, val_label_transform = dataset_and_transform_generate(clean_setting)

        if args.use_tiny_imagenet:
            tiny_clean_setting = copy.deepcopy(clean_setting)
            tiny_clean_setting.dataset = 'tiny'
            tiny_clean_setting.dataset_path = '/home/lupang/Documents/data/tiny'
            _, _, _, _, _, _, val_dataset_without_transform, _, _ = dataset_and_transform_generate(tiny_clean_setting)
            print("load tiny-imagenet as out-of-distribution data!!!!!!!!!!!!!!!!!!")
        elif args.use_gtsrb:
            gtsrb_clean_setting = copy.deepcopy(clean_setting)
            gtsrb_clean_setting.dataset = 'gtsrb'
            gtsrb_clean_setting.dataset_path = '/home/lupang/Documents/data/gtsrb'
            _, _, _, _, _, _, val_dataset_without_transform, _, _ = dataset_and_transform_generate(gtsrb_clean_setting)
            print("load gtsrb as out-of-distribution data!!!!!!!!!!!!!!!!!!")
        elif args.use_cifar10:
            cifar10_clean_setting = copy.deepcopy(clean_setting)
            cifar10_clean_setting.dataset = 'cifar10'
            cifar10_clean_setting.dataset_path = '/home/lupang/Documents/data/cifar10'
            _, _, _, _, _, _, val_dataset_without_transform, _, _ = dataset_and_transform_generate(cifar10_clean_setting)
            print("load cifar10 as out-of-distribution data!!!!!!!!!!!!!!!!!!")
        clean_train_ds = prepro_cls_DatasetBD(
            full_dataset_without_transform=train_dataset_without_transform,
            poison_idx=np.zeros(len(train_dataset_without_transform)),
            # one-hot to determine which image may take bd_transform
...
def load_attack_result_diversity(args,
    save_path : str,
):
    '''
    This function first replicate the basic steps of generate models and clean train and test datasets
    ...
    if all(key in load_file for key in ['model_name',
        ...
        if args.use_diversity:
            tiny_clean_setting = copy.deepcopy(clean_setting)
            tiny_clean_setting.dataset = 'diversity'
            tiny_clean_setting.dataset_path = os.path.join(args.diversity_path, args.diversity_type)
            val_dataset_without_transform = dataset_and_transform_generate_diversity(tiny_clean_setting)
            print("load diversity-imagenet as out-of-distribution data!!!!!!!!!!!!!!!!!!")
