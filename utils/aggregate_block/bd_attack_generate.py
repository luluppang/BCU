...
from utils.bd_img_transform.patch import AddMaskPatchTrigger, AddBlendedPatchTrigger, SimpleAdditiveTrigger
...
def bd_attack_img_trans_generate(args):
    ...
    if args.attack == 'fix_patch':
        ...
        bd_transform = AddMaskPatchTrigger(
            trans(np.load(args.patch_mask_path, allow_pickle=True).item().get("pattern")),
            np.load(args.patch_mask_path, allow_pickle=True).item().get("mask")
        )
    ...
    elif args.attack == 'blended_patch':
        trans = transforms.Compose([
            transforms.ToPILImage(),
            transforms.Resize(args.img_size[:2]),  # (32, 32)
            np.array,
        ...
        bd_transform = AddBlendedPatchTrigger(
            trans(np.load(args.patch_mask_path, allow_pickle=True).item().get("pattern")),
            np.load(args.patch_mask_path, allow_pickle=True).item().get("mask")
        )
        ...
        train_bd_transform = general_compose([
            ...
            float(args.attack_train_blended_alpha)), True) # 0.2,
        ])
        ...
        trigger_transform = AddMaskPatchTrigger(
            trans(np.load(args.patch_mask_path, allow_pickle=True).item().get("pattern")),
            np.load(args.patch_mask_path, allow_pickle=True).item().get("mask")
        )
...
def bd_attack_label_trans_generate(args):
    ...
    if args.attack_label_trans == 'all2one':
        ...
        bd_label_transform = AllToAll_shiftLabelAttack(
            int(args.attack_label_shift_amount), int(args.num_classes)
        )
