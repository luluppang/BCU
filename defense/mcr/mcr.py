...
def get_curve_class(args):
	...
	if args.model == 'preactresnet18':
		...
		net = getattr(curve_models, 'VGG16')
	elif args.model == 'vgg19':
		net = getattr(curve_models, 'VGG19BN')
	elif args.model == 'resnet18':
		net = getattr(curve_models, 'ResNet18')
...
def mcr(args, result, config):
	...
	for epoch in tqdm(range(start_epoch, args.ft_epochs)):
		...
		batch_loss = []
		for i, (inputs,labels) in enumerate(loaders['train']):  # type: ignore
			ft_model.train()
			ft_model.to(args.device)
			inputs, labels = inputs.to(args.device), labels.to(args.device)
	...
	print('the corresponding asr is', best_asr)
...
if __name__ == '__main__':
	...
	if args.log is None:
		...
		#args.log_file_name = args.result_file + '_' + str(args.random_seed)
		args.log_file_name = args.result_file[(args.result_file.rfind('/') + 1):] + '_' + str(args.random_seed)
