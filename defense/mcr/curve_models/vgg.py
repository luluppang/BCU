...
def make_layers(config, batch_norm=False, fix_points=None):
    ...
    for v in config:
        if v == 'M':
            ...
            conv2d = conv(in_channels, v, kernel_size=3, padding=1, **kwargs)
            if batch_norm:
                layers += [conv2d, bn(v, **kwargs), nn.ReLU(inplace=True)]
            else:
...
class VGGBase(nn.Module):
    def __init__(self, num_classes, depth=16, batch_norm=False):
        ...
        self.features = make_layers(config[depth], batch_norm)
...
class VGGCurve(nn.Module):
    def __init__(self, num_classes, fix_points, depth=16, batch_norm=False):
        ...
        # self.activation_blocks = activation_blocks
        # self.poolings = poolings
        self.features = make_layers(config[depth], batch_norm, fix_points=fix_points)
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
    ...
    def forward(self, x, coeffs_t):
        ...
        for layer in self.features:
            if isinstance(layer, nn.MaxPool2d) or isinstance(layer, nn.ReLU) or isinstance(layer, nn.Dropout):
                x = layer(x)
            else:
                x = layer(x, coeffs_t)
        # x = self.features(x)
